...
for(k in 1:K){
  if (is.numeric(dades[,k])){ 
    ...
    if(class(dd[,k])=="Date"){
      ...
      plot(marg,type="l",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]),col = "#FDAB9B")
      paleta<-rainbow(length(levels(dades[,k])))
      for(c in 1:length(levels(dades[,k]))){lines(colperc[,c],col= "#FDAB9B") }
      
      #with legend
      plot(marg,type="l",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]),col = "#FA8D76")
      paleta<-rainbow(length(levels(dades[,k])))
      for(c in 1:length(levels(dades[,k]))){lines(colperc[,c],col= "#FA8D76") }
      legend("topright", levels(dades[,k]), col=paleta, lty=2, cex=0.6)
      
      #condicionades a classes
      print(append("Categories=",levels(dades[,k])))
      plot(marg,type="n",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), col = "#F46C58")
      paleta<-rainbow(length(levels(dades[,k])))
      for(c in 1:length(levels(dades[,k]))){lines(rowperc[,c],col="#F46C58") }
      
      #with legend
      plot(marg,type="n",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), col = "#EB4F48")
      paleta<-rainbow(length(levels(dades[,k])))
      for(c in 1:length(levels(dades[,k]))){lines(rowperc[,c],col= "#EB4F48") }
      legend("topright", levels(dades[,k]), col=paleta, lty=2, cex=0.6)
      
      #amb variable en eix d'abcisses
      marg <-table(dades[,k])/n
      print(append("Categories=",levels(dades[,k])))
      plot(marg,type="l",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), las=3, col = "#E03A42")
      ...
      for(c in 1:length(levels(as.factor(P)))){lines(rowperc[c,],col="#E03A42") }
      
      #with legend
      plot(marg,type="l",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), las=3, col = "#E03")
      for(c in 1:length(levels(as.factor(P)))){lines(rowperc[c,],col="#E03")}
      legend("topright", levels(as.factor(P)), col=paleta, lty=2, cex=0.6)
      
      #condicionades a columna 
      plot(marg,type="n",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), las=3, col = "#D2293D")
      paleta<-rainbow(length(levels(as.factor(P))))
      for(c in 1:length(levels(as.factor(P)))){lines(colperc[c,],col="#D2293D") }
      
      #with legend
      plot(marg,type="n",ylim=c(0,1),main=paste("Prop. of pos & neg by",names(dades)[k]), las=3, col = "#BB173A")
      for(c in 1:length(levels(as.factor(P)))){lines(colperc[c,],col="#BB173A")}
      legend("topright", levels(as.factor(P)), col=paleta, lty=2, cex=0.6)
      ...
      paleta<-rainbow(length(levels(dades[,k])))
      barplot(table(dades[,k], as.factor(P)), beside=FALSE,col="#EB4F48" )
      
      barplot(table(dades[,k], as.factor(P)), beside=FALSE,col="#D2293D" )
      legend("topright",levels(as.factor(dades[,k])),pch=1,cex=0.5, col="#E03")
      
      #diagrames de barres adosades
      barplot(table(dades[,k], as.factor(P)), beside=TRUE,col="#BB173A")
      
      barplot(table(dades[,k], as.factor(P)), beside=TRUE,col="#FA8D76")
      legend("topright",levels(as.factor(dades[,k])),pch=1,cex=0.5, col= "#FDAB9B")
